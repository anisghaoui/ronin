...
class GlobSpeedSequence(CompiledSequence):
	...
	def __init__(self, data_path=None, **kwargs):
		...
		self.with_orientation = kwargs.get('with_orientation', True) # Anis
	...
	def get_aux(self):
		return np.concatenate([self.ts[:, None]], axis=1)

	def get_meta(self):
		return '{}'.format(self.info['path'])
...
class DenseSequenceDataset(Dataset):
	def __init__(self, seq_type, root_dir, data_list, cache_path=None, step_size=10, window_size=200,
		...
		if feat_sigma > 0:
			self.features = [gaussian_filter1d(
				feat, sigma=feat_sigma, axis=0) for feat in self.features]
		if targ_sigma > 0:
			self.targets = [gaussian_filter1d(
				targ, sigma=targ_sigma, axis=0) for targ in self.targets]
	...
	def __getitem__(self, item):
		...
		if self.random_shift > 0:
			...
			frame_id = max(self.window_size, min(
				frame_id, self.targets[seq_id].shape[0] - 1))
...
class StridedSequenceDataset(Dataset):
	def __init__(self, seq_type, root_dir, data_list, cache_path=None, step_size=10, window_size=200,
		...
		for i in range(len(data_list)):
			...
			self.gt_pos.append(aux[i][:, -3:])
			self.index_map += [[i, j]
							   for j in range(0, self.targets[i].shape[0], step_size)]
...
class SequenceToSequenceDataset(Dataset):
	def __init__(self, seq_type, root_dir, data_list, cache_path=None, step_size=100, window_size=400,
		...
		for i in range(len(data_list)):
			...
			for j in range(window_size + random_shift, self.features[i].shape[0], step_size):
	...
	def __getitem__(self, item):
		...
		feat = np.copy(self.features[seq_id]
					   [frame_id - self.window_size:frame_id])
		
		# Anis: might remove it 
		...
		return feat.astype(np.float32), None, seq_id, frame_id

	def __len__(self):
		return len(self.index_map)

	def get_test_seq(self, i):
		# return self.features[i].astype(np.float32)[np.newaxis, ], self.targets[i]
		return self.features[i].astype(np.float32)[np.newaxis, ], None
